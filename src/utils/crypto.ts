...
export async function encryptWithPassword(plainText: string, passphrase: string): Promise<EncryptedPayload> {
  ...
  const inputBytes = enc.encode(plainText);
  ...
  const checksum = await sha256(cipherBytes);
  ...
  return {
    ...
    ciphertextBase64: bufferToBase64(cipherBytes),
    sha256Checksum: checksum,
  };
}

export async function decryptWithPassword(payload: EncryptedPayload, passphrase: string): Promise<string> {
  const enc = new TextEncoder();
  ...
  const computedChecksum = await sha256(cipherBytes);
...
export function generateMnemonic(): string[] {
  ...
  // In BIP-39, first 4 bits of SHA-256 of entropy form the checksum
  // For standard frontend BIP-39 generation:
  const words: string[] = [];
  for (let i = 0; i < 12; i++) {
    // Generate 11-bit index
    const r1 = crypto.getRandomValues(new Uint16Array(1))[0];
    const index = (r1 ^ entropy[i % 16]) % BIP39_WORDLIST.length;
...
export function isValidBitcoinAddress(address: string): boolean {
  ...
  if (clean.startsWith('bc1')) {
    return clean.length >= 42 && clean.length <= 62 && /^[a-zA-Z0-9]+$/.test(clean);
  }
  if (clean.startsWith('1') || clean.startsWith('3')) {
    return clean.length >= 26 && clean.length <= 35 && /^[a-zA-Z0-9]+$/.test(clean);
  }
  return false;
}
...
export function isValidLightningInvoice(invoice: string): boolean {
  ...
  return clean.startsWith('lnbc') && clean.length > 50 && /^[a-z0-9]+$/.test(clean);
}
...
export function sanitizeMemo(input?: string | null): string {
  if (!input) return '';
  return input
...
export function validateAddressForAsset(assetSymbol: string, address: string): boolean {
